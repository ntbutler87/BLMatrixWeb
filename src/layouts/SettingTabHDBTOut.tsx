...
function SettingTabHDBTOutputs({matrixStatus, currentAppSettings}: Props): React.JSX.Element {
  
  return (
    <View style={{flex:1, flexDirection:'column', alignItems:'center'}}>
      <View style={styles.settingTileRow}>
      {currentAppSettings?.HDBT_OUT.map( (elem) => {
        return <SettingTilePort 
          key={"HDBT_OUT" + elem.port}
          title={"HDBaseT Output #" + elem.port}
          appPortConfig={elem}
          port={matrixStatus.HDBT_OUT[elem.port-1]}
          onPress={() => {}} 
        />
      } )}
      </View>
...
export default SettingTabHDBTOutputs;
