...
function SettingTabHDMIOutputs({matrixStatus, currentAppSettings}: Props): React.JSX.Element {
    ...
    <View style={{flex:1, flexDirection:'column', alignItems:'center'}}>
      ...
      {currentAppSettings?.HDMI_OUT.map( (elem) => {
        return <SettingTilePort 
          key={"HDMI_OUT" + elem.port}
          title={"HDMI Output #" + elem.port}
          appPortConfig={elem}
          port={matrixStatus.HDMI_OUT[elem.port-1]}
          onPress={() => {}} 
        />
      } )}
      </View>
...
export default SettingTabHDMIOutputs;
